'use client'
// src/components/lesson/LessonViewer.jsx — full rewrite
// Fixes: dark mode, back button, points bug, cleaner UI
...
function ExitModal({ onKeep, onExit }) {
  return (
    <div className="fixed inset-0 bg-black/60 z-[200] flex items-end sm:items-center justify-center px-4 pb-6 sm:pb-0"
      ...
      <div className="bg-card rounded-3xl w-full max-w-sm p-6 space-y-4 animate-in slide-in-from-bottom-4 duration-200 shadow-2xl">
        ...
        <div className="space-y-2.5">
          ...
          <button onClick={onExit}
            className="w-full py-3.5 border border-default text-primary text-sm font-medium rounded-2xl hover:bg-subtle transition-colors">
            Exit lesson
          </button>
        </div>
      </div>
...
export default function LessonViewer({ subtopic, userId, existingProgress }) {
  ...
  // POINTS FIX: awardPoints called here via PointsContext
  // This triggers toast + badge update. The /complete route handles streak.
  const handleComplete = useCallback(async () => {
    if (!userId) { setShowSignupPrompt(true); return }
    setCompleted(true)
    await saveProgress(totalSlides - 1, true)
    // Fire complete route (streak, daily count)
    try { await fetch(`/api/lessons/${subtopic.id}/complete`, { method: 'POST' }) } catch {}
    // Award points — PointsContext handles duplicate guard + toast
  ...
  if (completed && userId) {
    return (
      <div className="fixed inset-0 bg-base flex flex-col items-center justify-center px-6 z-50">
        <div className="max-w-sm w-full text-center space-y-5">
          ...
          <div className="space-y-3">
            ...
            <Link href="/student/dashboard"
              className="block w-full py-3.5 border border-default text-primary text-sm font-medium rounded-2xl hover:bg-subtle transition-colors">
              Back to dashboard
            </Link>
          </div>
        </div>
  ...
  return (
    <div className="fixed inset-0 bg-base flex flex-col z-50">
      ...
      <div className="flex-shrink-0 bg-card border-b border-default px-4 py-3">
        <div className="flex items-center gap-3 max-w-lg mx-auto">
          <button onClick={() => setShowExitModal(true)}
            className="w-8 h-8 rounded-xl bg-subtle flex items-center justify-center flex-shrink-0 hover:bg-indigo-50 dark:hover:bg-indigo-950/30 transition-colors">
          ...
          <div className="flex gap-2">
            {currentIndex > 0 && (
              <button
                ...
                className="px-5 py-4 border border-default text-secondary text-sm font-bold rounded-2xl hover:bg-subtle hover:text-primary transition-all flex-shrink-0">
                ← Back
              </button>
            )}
