...
function deriveActions({ learningPaths, lessonProgress, subtopicMap, max = 3 }) {
...
export default function TodaysFocus({ learningPaths, lessonProgress, subtopicMap }) {
  const actions = deriveActions({ learningPaths, lessonProgress, subtopicMap })
  if (!actions.length) return null

  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="font-black text-gray-900 text-sm">Today&apos;s Focus</h3>
        <span className="text-xs text-gray-400">{actions.length} suggested</span>
      </div>
        ...
        {actions.map((action, i) => (
          ...
          >
            ...
            <div className={`w-10 h-10 rounded-2xl ${action.color.bg} flex items-center justify-center flex-shrink-0`}>
              {action.type === 'lesson' ? (
                <svg className={`w-5 h-5 ${action.color.text}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm font-bold text-gray-900 truncate mt-0.5">{action.sublabel}</p>
            </div>

            <span className={`text-xs font-black px-3 py-1.5 rounded-xl flex-shrink-0 ${action.color.bg} ${action.color.text}`}>
              {action.cta} →
            </span>
          </Link>
        ))}
