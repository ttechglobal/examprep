'use client'
// src/components/games/BuildItEngine.jsx
// REDESIGNED — same fix pattern, amber theme for Build It game type.

import { useState, useMemo } from 'react'
import { getGameTypeTheme, getResultTheme } from '@/lib/gameTheme'
...
export default function BuildItEngine({ game, onComplete }) {
  const isDark = useIsDark()
  const theme  = getGameTypeTheme('build_it')
  ...
  const softBorder = isDark ? theme.darkBorder : theme.border
  ...
  const trackBg    = isDark ? '#374151' : '#e2e8f0'
  ...
  if (phase === 'done') {
    ...
    return (
      <div className="space-y-5">
        <div className="rounded-2xl px-5 py-4 text-center" style={{ background: softBg, border: `1.5px solid ${softBorder}` }}>
        ...
        <div className="flex flex-col items-center gap-2">
          <p className="text-4xl font-black" style={{ color: isDark ? resultTheme.darkSolid : resultTheme.solid }}>{pct}%</p>
        ...
        <div className="space-y-2">
          {current.options.map(opt => {
            ...
            const style = !revealed
              ...
              : isCorrect
              ? { background: isDark ? correctTheme.darkBg : correctTheme.bg, border: `2px solid ${isDark ? correctTheme.darkBorder : correctTheme.border}` }
              : isSelected
              ? { background: isDark ? wrongTheme.darkBg : wrongTheme.bg, border: `2px solid ${isDark ? wrongTheme.darkBorder : wrongTheme.border}` }
            ...
            return (
              <button
                ...
                disabled={revealed}
                style={{ ...style, color: isDark ? '#f9fafb' : '#111827' }}
                className="w-full text-left px-4 py-3 rounded-xl text-sm font-bold transition-all"
              >
                {revealed && isCorrect && <span className="mr-2" style={{ color: isDark ? correctTheme.darkSolid : correctTheme.solid }}>✓</span>}
                {revealed && isSelected && !isCorrect && <span className="mr-2" style={{ color: isDark ? wrongTheme.darkSolid : wrongTheme.solid }}>✗</span>}
                {opt}
              </button>
            )
          })}
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl px-4 py-3" style={{ background: softBg, border: `1.5px solid ${softBorder}` }}>
      ...
      {/* Insight */}
      <div className="flex items-start gap-2.5 px-4 py-3 rounded-2xl" style={{ background: softBg, border: `1.5px solid ${softBorder}` }}>
