  ...
  const handleSignup = async (e) => {
    ...
    const { data, error: signupError } = await supabase.auth.signUp({
      ...
      options: {
        ...
        emailRedirectTo: `${window.location.origin}/api/auth/callback?next=/school/onboarding`,
      },
    })
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 bg-indigo-600 text-white text-sm font-black rounded-xl hover:bg-indigo-500 disabled:opacity-50 transition-colors"
            >
        ...
        <p className="text-center text-xs text-gray-400 mt-4">
          Setting up for students instead?{' '}
