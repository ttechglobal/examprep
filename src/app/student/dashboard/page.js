...
// Weekly Goals ABOVE Practice Hub. No XP. Dark mode via Tailwind dark: classes.
...
const PracticeCTA = memo(function PracticeCTA() {
  return (
    <Link href="/student/practice"
      className="relative overflow-hidden flex items-center gap-4 bg-white dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm px-5 py-4 hover:shadow-md hover:border-indigo-100 dark:hover:border-indigo-800 transition-all group active:scale-[0.98]">
      <div className="absolute right-0 top-0 bottom-0 w-20 bg-gradient-to-l from-indigo-50 dark:from-indigo-950/20 to-transparent rounded-r-3xl pointer-events-none" />
...
const GoalBanner = memo(function GoalBanner({ profile, onClick }) {
  ...
  if (!profile?.goals_set) {
    return (
      <button onClick={onClick}
        className="w-full flex items-center gap-4 bg-gradient-to-r from-amber-400 to-orange-500 rounded-3xl px-5 py-4 text-left hover:opacity-95 active:scale-[0.98] transition-all shadow-lg shadow-orange-100 dark:shadow-orange-900/20">
        <span className="text-2xl">🎯</span>
        <div className="flex-1">
          <p className="font-black text-white text-sm">Set your exam goals</p>
          <p className="text-orange-100 text-xs mt-0.5">Target score, course, university</p>
        </div>
  ...
  return (
    <button onClick={onClick}
      className="w-full flex items-center gap-4 bg-white dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm px-5 py-4 text-left hover:shadow-md hover:border-indigo-100 dark:hover:border-indigo-700 transition-all active:scale-[0.98]">
...
// ─── Swipeable subjects ───────────────────────────────────────────────────────
const SubjectSlider = memo(function SubjectSlider({ learningPaths, lessonProgress, subtopicMap }) {
  const scrollRef  = useRef(null)
  const [activeIdx, setActiveIdx] = useState(0)

  const completedIds = useMemo(() =>
    new Set((lessonProgress ?? []).filter(p => p.completed).map(p => p.subtopic_id)),
  [lessonProgress])
  ...
  function onScroll() {
    ...
    const cardW = el.clientWidth * 0.72
    setActiveIdx(Math.min(Math.round(el.scrollLeft / cardW), learningPaths.length - 1))
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <Link href="/student/learn" className="text-xs font-bold text-indigo-500">See all →</Link>
      </div>
        ...
        {learningPaths.map((path, i) => {
          ...
          return (
            <div key={path.subject_id} className="flex-shrink-0 snap-start w-[72vw] max-w-[280px]">
              <div className={`${color.bg} rounded-3xl p-5 h-full`}>
                ...
                {nextSub && (
                  <p className={`text-xs ${color.text} opacity-80 mb-4 truncate`}>
                    <span className="font-bold">Next:</span> {nextSub.name}
                  </p>
                )}
...
const StudyPlanCard = memo(function StudyPlanCard({ learningPaths, subtopicMap, completedIds }) {
  ...
  const pct    = allIds.length > 0
    ? Math.round((allIds.filter(id => completedSet.has(id)).length / allIds.length) * 100) : 0
    ...
    <div className="bg-white dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden">
      <div className="flex items-center justify-between px-5 pt-4 pb-3">
        <div>
          ...
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{pct}% complete</p>
        </div>
      ...
      {next && (
        <Link href={`/student/lesson/${next.id}`}
          className="flex items-center gap-3 px-5 py-3.5 bg-indigo-50 dark:bg-indigo-950/20 border-t border-indigo-100 dark:border-indigo-900/50 hover:bg-indigo-100 dark:hover:bg-indigo-950/40 transition-colors">
          ...
          <svg className="w-4 h-4 text-indigo-300 dark:text-indigo-700 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
          </svg>
        </Link>
      )}
...
export default function StudentDashboard() {
  ...
  async function loadLearningData(userId, prog = []) {
    ...
    if (!prog.length) {
      const { data: fp } = await supabase
        .from('lesson_progress').select('subtopic_id, completed').eq('student_id', userId).in('subtopic_id', allIds)
  ...
  return (
    <div className="space-y-4">
      ...
      {hasPath && (
        <>
          {/* 1. Weekly Goals — above Practice Hub */}
          <WeeklyGoals />
        ...
        <>
          <div className="bg-white dark:bg-gray-900 rounded-3xl border border-indigo-100 dark:border-indigo-900 shadow-sm p-6">
            ...
            <Link href="/diagnostic"
              className="block w-full py-3.5 bg-gradient-to-r from-indigo-600 to-violet-600 text-white text-sm font-black rounded-2xl hover:opacity-90 text-center mb-3">
              Take the diagnostic →
            </Link>
            <Link href="/student/learn"
              className="block text-center text-sm text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-400 font-medium">
              Or browse lessons directly →
            </Link>
          </div>

          {/* Weekly Goals above Practice Hub here too */}
          <WeeklyGoals />
          <PracticeCTA />
          <GoalBanner profile={profile} onClick={() => setShowGoalModal(true)} />
        </>
