...
export async function POST(request) {
  ...
  const { data: accessCode } = await db
    .from('access_codes')
    .select('id, type, duration_days, max_uses, uses_count, is_active, expires_at, school_id, schools(name)')
    .eq('code', normalised)
    .single()
  ...
  const schoolName = accessCode.schools?.name ?? null

  const profileUpdate = { premium_until: accessExpiresAt }
  if (accessCode.school_id) {
    profileUpdate.school_id   = accessCode.school_id
    profileUpdate.school_name = schoolName  // Issue 2: was never written
  }

  // Issue 7: cohort_id was inserted into cohort_members but never written to profiles.
  // The join route (/api/school/join) sets both — redeem must match.
  let cohortId = null
  if (accessCode.school_id) {
    ...
    if (activeCohort) {
      ...
      profileUpdate.cohort_id = activeCohort.id
  ...
  return NextResponse.json({
    ...
    school_name:      schoolName,   // Issue 2: now returned so UI can show it immediately
  })
}
