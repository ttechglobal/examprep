...
function Input({ type='text', value, onChange, placeholder, autoComplete, autoFocus }) {
  return (
    <input type={type} value={value} onChange={onChange} placeholder={placeholder}
      autoComplete={autoComplete} autoFocus={autoFocus}
      style={{ width:'100%', padding:'12px 14px', borderRadius:12, boxSizing:'border-box', border:`1.5px solid ${value ? NAVY : BORDER}`, background:'#f7f8fc', fontSize:14, color:TEXT, outline:'none', fontFamily:'inherit', transition:'border-color .15s' }}
    />
  )
}
